#!/usr/bin/env python3
"""
Run rule-based cultural classification on the test set and save predictions.
"""
import argparse
import ast
...
def main():
    parser = argparse.ArgumentParser(
        description='Rule-based cultural classification on test data'
    )
    parser.add_argument(
        '-i', '--input',
        type=Path,
        default=Path('data/test_enriched.csv'),
        help='Path to enriched test CSV'
    )
    parser.add_argument(
        '--golden-instance',
        type=Path,
    ...
    parser.add_argument(
        ...
        help='Directory to save rule-based predictions'
    )
    args = parser.parse_args()

    args.output_dir.mkdir(parents=True, exist_ok=True)
    output_file = args.output_dir / 'test_rules.csv'
    ...
    tqdm.pandas(desc='Applying rule-based classifier')
    ...
    df[['qid', 'prediction', 'source']].to_csv(output_file, index=False)
    print(f'Rule-based predictions saved to: {output_file}')
